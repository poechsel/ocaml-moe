...
type next_points_status = {
  ...
  optimizer_success : Yojson.Safe.json (* for now *);
}
[@@deriving yojson]
...
struct
  ...
  type request = {
    ...
    optimizer_info : Arg.parameters optimizer_info (* has empty default value *);
  ...
  let create
    ...
    ?(optimizer_info = default_optimizer_info)
    ?(points_being_sampled = [])
    ?(mvndst_parameters = default_mvndst_parameters)
    ~gp_historical_info
    ~domain_info ()
    =
    {
  ...
  type response = next_points_response
  [@@deriving yojson]
end

module ConstantLiar(Arg : Optimizer)  =
struct
