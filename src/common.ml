...
type point_sampled = {point_sampled : single_point}
[@@deriving yojson]
type points_sampled = point_sampled list
[@@deriving yojson]

type domain_coordinate = {
  min : float;
  max : float;
}[@@deriving yojson]
...
type mvndst_parameters = {
  releps : float;
  abseps : float;
  maxpts_per_dim: int;
} [@@deriving yojson]

let default_mvndst_parameters = {
  releps = 1.0e-9;
  abseps = 1.0e-9;
...
type covariance_type = SQUARE_EXPONENTIAL_COVARIANCE_TYPE  [@name "square_exponential"]
[@@deriving yojson]

let square_exponential_covariance_type = "square_exponential"

type covariance_info = {
  covariance_type : string(*covariance_type*);
  hyperparameters : float list
}[@@deriving yojson]

let default_covariance_info = {
  covariance_type = square_exponential_covariance_type(*SQUARE_EXPONENTIAL_COVARIANCE_TYPE*);
  hyperparameters = []
}
[@@deriving yojson]
...
type gp_historical_info = {
  points_sampled : points_sampled list
}[@@deriving yojson]

type list_of_points_in_domain_item = {
  points_in_domain : point
}
[@@deriving yojson]
...
type list_of_expected_improvements_item = {
...
type matrix_of_floats_item = {
  row_of_matrix : float list
}
[@@deriving yojson]
type matrix_of_floats = matrix_of_floats_item list
[@@deriving yojson]
